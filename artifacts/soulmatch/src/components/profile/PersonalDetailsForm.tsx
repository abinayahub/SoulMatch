  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="premium-glass-card rounded-[24px] p-4 sm:p-6 mb-4 border border-white/50">
      <div className="mb-5 border-b border-white/40 pb-3 text-center">
        <h2 className="text-[20px] sm:text-[22px] font-black mb-2 text-[#4A3B3B]">Personal Details</h2>
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="space-y-1">
          <Label className="text-[12px] font-bold text-[#4A3B3B] uppercase tracking-wider ml-1">Full Name <span className="text-[#FF7A7A]">*</span></Label>
          <div className="relative">
            <User className="absolute z-10 left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[#C8B8B8]" />
            <Input className={`h-[50px] pl-10 text-[15px] text-[#252525] placeholder:text-[#B8A8A8] focus-visible:ring-[#FF9A9A]/50 bg-white/60 ${form.formState.errors.fullName ? "border-red-500" : "border-white/50"}`} placeholder="e.g. Aria Sharma" {...form.register("fullName", { required: "Name is required" })} />
          </div>
        ...
        {/* Date of Birth */}
        <div className="space-y-1">
          <Label className="text-[12px] font-bold text-[#4A3B3B] uppercase tracking-wider ml-1">Date of Birth <span className="text-[#FF7A7A]">*</span></Label>
          <div className="relative">
            <Calendar className="absolute z-10 left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[#C8B8B8]" />
            <Input type="date" className={`h-[50px] pl-10 pr-4 text-[15px] text-[#252525] focus-visible:ring-[#FF9A9A]/50 bg-white/60 ${form.formState.errors.dateOfBirth ? "border-red-500" : "border-white/50"}`} {...form.register("dateOfBirth", { required: "Birthday is required" })} />
          </div>
          ...
          <p className="text-[11px] text-[#8A7A7A] ml-1">Used to calculate your age. Cannot be changed later.</p>
        </div>

        {/* Gender Choice Chips */}
        <div className="space-y-2">
          <Label className="text-[12px] font-bold text-[#4A3B3B] uppercase tracking-wider ml-1">Gender <span className="text-[#FF7A7A]">*</span></Label>
        ...
        <div className="space-y-2">
          <Label className="text-[12px] font-bold text-[#4A3B3B] uppercase tracking-wider ml-1">Relationship Status <span className="text-[#FF7A7A]">*</span></Label>
          <div className="flex flex-wrap gap-2">
            {statuses.map((s) => {
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-1">
            <Label className="text-[12px] font-bold text-[#4A3B3B] uppercase tracking-wider ml-1">Height (cm) <span className="text-[#FF7A7A]">*</span></Label>
            <div className="relative">
              <Ruler className="absolute z-10 left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[#C8B8B8]" />
              <Input type="number" className={`h-[50px] pl-10 text-[15px] text-[#252525] placeholder:text-[#B8A8A8] focus-visible:ring-[#FF9A9A]/50 bg-white/60 ${form.formState.errors.height ? "border-red-500" : "border-white/50"}`} placeholder="170" {...form.register("height", { required: "Height is required" })} />
            </div>
            {form.formState.errors.height && <p className="text-xs text-red-500 ml-1">{form.formState.errors.height.message as string}</p>}
            <p className="text-[11px] text-[#8A7A7A] ml-1">Improves match accuracy.</p>
          </div>
          
          <div className="space-y-1">
            <Label className="text-[12px] font-bold text-[#4A3B3B] uppercase tracking-wider ml-1">Weight (kg)</Label>
            <div className="relative">
              <Scale className="absolute z-10 left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[#C8B8B8]" />
              <Input type="number" className="h-[50px] pl-10 text-[15px] text-[#252525] placeholder:text-[#B8A8A8] focus-visible:ring-[#FF9A9A]/50 bg-white/60 border-white/50" placeholder="65" {...form.register("weight")} />
            </div>
            <p className="text-[11px] text-[#8A7A7A] ml-1">Optional field.</p>
          </div>
        </div>

        {/* Bio */}
        <div className="space-y-1">
          <Label className="text-[12px] font-bold text-[#4A3B3B] uppercase tracking-wider ml-1">Bio <span className="text-[#FF7A7A]">*</span></Label>
          <Textarea 
            className={`min-h-[80px] p-4 rounded-[16px] text-[14px] leading-relaxed resize-none text-[#252525] placeholder:text-[#B8A8A8] focus-visible:ring-[#FF9A9A]/50 bg-white/70 shadow-sm border ${form.formState.errors.bio ? "border-red-500" : "border-white/60"}`} 
          ...
          <p className="text-[10px] text-[#8A7A7A] ml-1">Write 2–3 sentences about yourself.</p>
        </div>
