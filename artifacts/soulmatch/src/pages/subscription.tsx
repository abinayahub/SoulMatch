      ...
      <div className="absolute inset-0 opacity-40 pointer-events-none" style={{ background: 'radial-gradient(circle at 0% 0%, #F4F1FF 0%, transparent 50%), radial-gradient(circle at 100% 100%, #FFFDFC 0%, transparent 50%)' }} />
        <div className="max-w-md mx-auto px-5 py-6">
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
            <h1 className="text-[clamp(24px,7.12vw,32px)] font-extrabold flex items-center gap-3 mb-1 text-[#252525]">
              <CreditCard className="w-7 h-7 text-primary" />Subscription
            </h1>
            <p className="text-[#707070] font-medium text-[clamp(13px,3.82vw,17px)]">Manage your plan and rewards.</p>
          </motion.div>
          ...
          <div className="space-y-6">
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-card border border-border shadow-sm rounded-[32px] p-6 relative overflow-hidden">
              ...
              <h2 className="text-[clamp(11px,3.31vw,15px)] font-extrabold text-[#707070] uppercase tracking-widest mb-4">Current Plan</h2>
              {currentSub ? (
                <div className="relative z-10">
                  <div className="flex items-center gap-2 mb-3">
                    <Crown className="w-6 h-6 text-accent" />
                    <span className="font-extrabold text-[clamp(19px,5.60vw,25px)] text-[#252525]">{(currentSub as any).plan?.name}</span>
                  ...
                  <div className="flex items-center gap-2 text-[clamp(12px,3.56vw,16px)] font-bold text-foreground/80 mb-6 bg-foreground/5 p-3 rounded-2xl">
                  ...
                  {(currentSub as any).cancelAtPeriodEnd && (
                    <div className="flex items-center justify-center p-3 rounded-2xl bg-yellow-500/10 text-yellow-600 font-bold text-[clamp(11px,3.31vw,15px)]">
                ...
                <div className="relative z-10">
                  <div className="flex items-center gap-2 mb-4">
                    <span className="font-extrabold text-[clamp(19px,5.60vw,25px)] text-[#252525]">Free Plan</span>
                  </div>
                  <div className="space-y-3 mb-6 bg-foreground/5 p-4 rounded-2xl">
                    {["Limited profile views", "10 interests/day", "No chat access"].map((f) => (
                      <div key={f} className="flex items-center gap-3 text-[clamp(12px,3.56vw,16px)] font-medium text-[#252525]">
            ...
            {rewards && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="bg-card border border-border shadow-sm rounded-[32px] p-6">
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-[clamp(11px,3.31vw,15px)] font-extrabold text-[#707070] uppercase tracking-widest">Rewards</h2>
                  <div className={`px-3 py-1 rounded-full border text-[clamp(9px,2.80vw,13px)] font-bold uppercase tracking-wider ${tierColors[(rewards as any).tier ?? "bronze"]}`}>
                    {((rewards as any).tier ?? "bronze")} Tier
                  </div>
                </div>
                <div className="flex items-end gap-2 mb-6">
                  <Sparkles className="w-8 h-8 text-primary mb-1" />
                  <span className="text-[clamp(34px,10.18vw,46px)] font-extrabold text-[#252525] leading-none">{(rewards as any).coins ?? 0}</span>
                  <span className="text-[clamp(12px,3.56vw,16px)] font-bold text-[#707070] mb-1">SoulCoins</span>
                </div>
                
                <h3 className="text-[clamp(10px,3.05vw,14px)] font-bold text-[#252525] mb-3">Recent Activity</h3>
                <div className="space-y-2">
                  {((rewards as any).recentTransactions ?? []).slice(0, 3).map((t: any) => (
                    <div key={t.id} className="flex items-center justify-between p-3 bg-foreground/5 rounded-2xl">
                      <span className="text-[clamp(11px,3.31vw,15px)] font-medium text-[#252525] truncate mr-4">{t.description}</span>
                      <span className={`text-[clamp(12px,3.56vw,16px)] font-extrabold shrink-0 ${t.type === "earned" ? "text-green-500" : "text-red-500"}`}>
                        {t.type === "earned" ? "+" : "-"}{t.amount}
                      </span>
                    </div>
                  ))}
                  {((rewards as any).recentTransactions ?? []).length === 0 && (
                     <p className="text-[clamp(11px,3.31vw,15px)] text-[#707070] italic p-2">No recent activity.</p>
                  )}
                </div>
              </motion.div>
            )}
            ...
            {/* Available Plans */}
            <div>
              <h2 className="text-[clamp(15px,4.58vw,21px)] font-extrabold text-[#252525] mb-4 pl-1">Available Plans</h2>
              {loadingPlans ? (
                    ...
                    >
                      {plan.isPopular && (
                        <div className="absolute -top-3.5 left-6">
                          <Badge className="bg-primary text-primary-foreground shadow-md border-0 text-white text-[clamp(9px,2.54vw,12px)] uppercase font-bold tracking-wider px-3 py-1 rounded-full">Most Popular</Badge>
                        </div>
                      )}
                      
                      <div className="mb-5">
                         <h3 className="font-extrabold text-[clamp(17px,5.09vw,23px)] text-[#252525] mb-1">{plan.name}</h3>
                         <div className="flex items-baseline gap-1">
                           <span className="text-[clamp(24px,7.12vw,32px)] font-extrabold text-primary">${plan.price}</span>
                           <span className="text-[#707070] font-bold text-[clamp(12px,3.56vw,16px)]">/{plan.interval}</span>
                         </div>
                      </div>
                      ...
                      <div className="space-y-2.5 mb-6">
                        {plan.features?.map((f: string) => (
                          <div key={f} className="flex items-start gap-3">
                            ...
                            <span className="text-[clamp(11px,3.31vw,15px)] font-medium text-[#252525] leading-snug">{f}</span>
                          </div>
                        ))}
                      </div>
                      ...
                      <Button
                        ...
                        className={`w-full h-12 rounded-2xl font-bold text-[clamp(13px,3.82vw,17px)] transition-all ${(currentSub as any)?.planId === plan.id ? "bg-green-500/10 text-green-600 hover:bg-green-500/20 border-0" : plan.isPopular ? "bg-primary text-primary-foreground shadow-lg shadow-primary/25 border-0 text-white" : "bg-foreground/5 hover:bg-foreground/10 text-[#252525]"}`}
