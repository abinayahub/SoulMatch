...
async function main() {
  // 1. Update Mani to be female
  await db.update(usersTable).set({ gender: "female" }).where(eq(usersTable.id, 4));
  console.log("Updated Mani to female.");

  // 2. Delete all existing matches (since they are now all female-female)
  await db.delete(interestsTable);
  console.log("Deleted old invalid matches.");

  // 3. Insert a couple of male users
  const [rahul] = await db.insert(usersTable).values({
    firstName: "Rahul",
    lastName: "S",
    email: "rahul.test@soulmatch.ai",
  ...
  const [vikram] = await db.insert(usersTable).values({
    firstName: "Vikram",
    lastName: "K",
    email: "vikram.test@soulmatch.ai",
    passwordHash: "mock_hash",
    gender: "male",
