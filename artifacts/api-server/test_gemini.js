...
var analyzeStory = async (stories) => {
...
"${s}"`).join("\n\n");
    const prompt = `Analyze the following series of chronological journal entries written by the same user to determine their behavioral traits.
CRITICAL INSTRUCTIONS - STRICT ADHERENCE REQUIRED:
1. EXPLICIT EVIDENCE ONLY: You must ONLY assign High, Medium, or Low if there is direct, explicit evidence in the text.
2. DEFAULT TO UNKNOWN: If the entries lack explicit evidence for a category, you MUST return "Unknown".
3. NO OVER-INFERENCING: Do NOT infer family values, career focus, or relationship commitment from unrelated activities (e.g., outdoor activities, hiking, or hobbies alone). 
4. PER-TRAIT CONFIDENCE SCORE: Provide a 'confidence' score (0-100) for EACH trait individually. If evidence for a specific trait is limited or sparse, its confidence MUST decrease significantly.
5. SUMMARY RULES: The summary MUST ONLY mention traits and conclusions that are directly and explicitly supported by the journal text. Do not hallucinate or extrapolate.
...
${combinedStories}`;
    ...
    return { summary: "AI insights are currently unavailable." };
  }
};

// test_gemini.ts
analyzeStory(["Navigating dating in a new city. Moved to a new place and used the app to meet people. It's a wonderful journey so far."]).then((res) => console.log(JSON.stringify(res, null, 2))).catch(console.error);
