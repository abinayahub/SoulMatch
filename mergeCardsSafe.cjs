...
const profileCardEndStr = '{/* Interest Compatibility Analysis */}';

if (content.includes(profileCardStartStr) && content.includes(profileCardEndStr)) {
    const pEndIndex = content.indexOf(profileCardEndStr);
    
    // We need to delete the original Profile Card which is INSIDE the grid.
    // The Profile Card starts at `{/* Profile Card */}` and ends at `</div>\n            {/* Right Column:`
    const rightColStr = '{/* Right Column: Compatibility & Analysis */}';
    if (content.includes(rightColStr)) {
        let profileCardJSX = content.substring(
            content.indexOf(profileCardStartStr), 
            content.indexOf(rightColStr)
        );

        // Remove the trailing `</div>` and other wrapper stuff from profileCardJSX.
        const lastDivMatch = profileCardJSX.lastIndexOf('</div>');
        if (lastDivMatch !== -1) {
            profileCardJSX = profileCardJSX.substring(0, lastDivMatch);
        }
...
` + profileCardJSX + '\n' + compJSX + `
            ...
            // Delete the old compJSX
            content = content.substring(0, compStart) + content.substring(compEnd);

            // Delete the old profileCardJSX by removing from profileCardStartStr up to rightColStr
            const rightColIndex = content.indexOf(rightColStr);
            const profCardIndex = content.indexOf(profileCardStartStr);
            if (profCardIndex !== -1 && rightColIndex !== -1) {
               content = content.substring(0, profCardIndex) + content.substring(rightColIndex);
            }

            // Insert the unified hero right BEFORE the grid
...
// 5. Wrap the Detailed Analysis Sections with a premium check!
const detailedStartStr = '{/* Detailed Analysis Sections */}';
if (content.includes(detailedStartStr)) {
    const detailedStart = content.indexOf(detailedStartStr);
    const wrapperStart = `
              {(user?.role === 'premium' || user?.role === 'admin') && (
                <>
    `;
    content = content.substring(0, detailedStart) + wrapperStart + content.substring(detailedStart);
    
    // Find the end of the analysis sections. It's right before `</div>\n          </div>\n        )}`
    // Which is before the final closing tags of the entire page.
...
console.log('Successfully merged safely!');
